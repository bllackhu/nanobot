...
>
> Token-driven consolidation makes two LLM calls when it hides turns: a SNIP fact extract appended to `memory/history.jsonl` (Dream intake), and a working **session checkpoint** stored in session metadata and injected as `[Session Checkpoint]` on the same turn. SNIP is long-term memory material (preferences, durable project facts). The checkpoint is how to continue this chat (goal, progress, next steps). They are not the same document.
...
| `agents.defaults.idleNewSessionHintAfterHours` | `0` | Hours since the last assistant reply before sending an idle reminder on the next user turn. `0` disables (the default). Fractions are allowed (for example `0.1` is 6 minutes). Changing this requires a gateway restart. |
...
1. **`"live"` (default)** — a separate CardKit **live progress card** whose single line is **replaced** on each tool call (latest overrides previous after the previous line's capped typewriter window), so users see progress without chat spam. A multi-tool batch plays sequentially on that one line; with `liveToolHintPrintStrategy: "fast"` and `liveToolHintTypewriterCapMs` (default 100), long hints dwell at most the cap then dump leftover characters on the next write. Tools are not stacked on the card. Each tool-hint line carries a trailing note (`🔧 read docs/api.md - processing`) and stays on the card (with the heartbeat pulse) until the tool **batch** finishes; then the note flips to `- done` and holds for `liveToolHintDoneHoldMs` before `AI thinking` can take over for the next LLM wait. When the turn ends the card is finalized (streaming closed); leftover `processing` still flips to `- done`. It also shows token-consolidation status while the agent is compacting history, and an `AI thinking ...` status line while the agent is waiting on the LLM (both status lines keep their own text and do not get the processing note). When a turn ends on `AI thinking`, the card freezes as `AI thinking - done`. The card is never left pulsing after the turn ends.
...
| `channels.feishu.liveToolHintHeartbeatSeconds` | `0.3` | How often (seconds) the live card sleeps between pulse ticks while a long-running step produces no new hint. Thinking and tool lines share a two-frame middle-dot blink (`··` ↔ `·····`). The first tick is scheduled immediately, but pulses are deferred while the last hint line is still inside the (capped) typewriter window so leftover characters can dump via `fast` before pulse dots land; later ticks wait this interval, but the visible floor is `max(interval, CardKit RTT)` because each tick awaits a sequential CardKit write. Cadence is a fixed monotonic schedule: missed frames are skipped rather than bursting catch-up writes. On in-flight tool lines and `consolidating history`, each tick also inserts a one-decimal elapsed label (`3.0s`) between the line and the pulse once `liveToolHintElapsedAfterMs` is reached. `0` disables the heartbeat (pulse and elapsed counter). Only applies in `hintMode: "live"`. |
...
| `channels.feishu.liveToolHintDoneNote` | `"done"` | Note applied when a tool **batch** finishes, and again if the live card is finalized at the end of a turn. Tool lines swap `processing` → this note (e.g. `🔧 read docs/api.md - done`). A leftover thinking line at turn-end strips trailing periods and appends it (`AI thinking ...` → `AI thinking - done`). Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintPrintFrequencyMs` | `1` | Typewriter animation speed for the live card line (Feishu `print_frequency_ms`): delay between printed characters, in ms. Range 1–70 (Feishu default is 70). Lower = faster animation. Only takes effect on Feishu clients **v7.23+**; older clients use the platform default. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintPrintStep` | `1` | Typewriter step size for the live card line (Feishu `print_step`): characters printed per tick. Range 1–20 (Feishu default is 1). Higher = faster animation. Same v7.23+ client caveat. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintPrintStrategy` | `"fast"` | Typewriter strategy (Feishu `print_strategy`): `"fast"` pre-flushes unfinished text before the next CardKit update (so leftover characters dump when the next tool or heartbeat pulse arrives after the dwell cap); `"delay"` waits for the typewriter to finish and can feel slow on long lines. Heartbeat pulses are also deferred until the local (capped) typewriter window ends. This does **not** control thinking→tool takeover. With `"fast"`, non-prefix live updates skip the one-char stream-reset seed (avoids a stuck wrench icon); `"delay"` still seeds then writes the full line so typewriter can continue. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintTypewriterCapMs` | `100` | Max dwell (ms) for the sequential typewriter wait / busy window before the next live-card write. The wait is `min(full typewriter estimate, cap)`. With `printStrategy: "fast"`, leftover characters dump on that next write so a long hint still appears in full inside the cap. `0` disables the cap (full estimate). Range 0–2000. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintMinDwellMs` | `800` | Extra glance time (ms) after the typewriter window before the next tool hint, heartbeat pulse, finish-to-done swap, or `AI thinking` may replace the current line. The glance floor starts when the CardKit write returns, so a `fast` leftover dump cannot beat the dwell. Combined default (`100` cap + `800` dwell = 900ms) means the first pulse lands after the glance window. `0` disables the extra wait (typewriter-only). Range 0–2000. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintDoneHoldMs` | `200` | How long (ms) to keep the static `… - done` line after a tool batch finishes before `AI thinking` may replace it. Heartbeat is cancelled for that hold so done does not pulse. `0` skips the extra hold (thinking still waits for any remaining typewriter window). Range 0–2000. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintElapsedAfterMs` | `3000` | Hide the live-card elapsed label until the currently visible in-progress line (in-flight tool, `consolidating history`, or `AI thinking`) has been on the card at least this long (ms). Heartbeat then inserts a one-decimal label (`3.0s`, `3.3s`) between the line and the pulse. Short waits stay `🔧 read foo.md - processing ··` / `AI thinking ··` with no timer; `history consolidated` and done lines stay without a timer. `0` shows from the first heartbeat after the line is committed (still omits a literal `0.0s`). Heartbeat `0` disables both pulse and counter. Range 0–600000. Only applies in `hintMode: "live"`. |
| `channels.feishu.liveToolHintMaxConsecutiveFailures` | `5` | Circuit breaker for the live-card heartbeat. After this many **consecutive** failed heartbeat updates (range 1–50), the card is treated as dead — its buffer is dropped and the heartbeat stops, so a dead card cannot burn API quota. A single successful update resets the counter, so a transient blip does not kill a healthy card. `code=200850` (card streaming timeout) and `code=300309` (streaming mode closed) retire the card on the first response: the buffer is dropped, the heartbeat is cancelled, and that `card_id` is not written again (a later hint in the same turn may create a new card). Those codes skip the internal reopen, so streaming mode is not turned back on and Feishu's 10-minute window is not restarted. Other failures still use this consecutive-failure limit. Turn-end finalize also retires a live card stored under the trigger `message_id` when the final outbound has none (typical of cron). Only applies in `hintMode: "live"`. |

In `live` mode the card also shows **status lines**: while the agent waits on the model it emits `AI thinking ...` (shown on the card as `AI thinking` with the same `··` ↔ `·····` pulse as tool hints), and during token-consolidation it emits `consolidating history (N/M tokens)` with a final `history consolidated` line. Heartbeat only refreshes the current line; it does **not** invent thinking or switch thinking↔tool. `AI thinking` is a real LLM-wait status emitted at the start of each agent iteration. A tool hint taking over from thinking cancels that pulse and writes the full tool line (`fast` pre-flushes the previous text without a one-char seed; `delay` still seeds then writes the full line). The tool line stays on `- processing` (with pulse) until the tool **batch** finishes — thinking is dropped while tools are in flight, even after the typewriter cap. Long-running in-flight tools, `consolidating history`, and `AI thinking` also show a one-decimal elapsed label on heartbeat ticks after `liveToolHintElapsedAfterMs` (default 3s); `history consolidated` and done lines do not. On finish the card waits out any remaining glance dwell, then swaps to `- done`, cancels the pulse, holds for `liveToolHintDoneHoldMs`, then the next LLM wait may show `AI thinking`. Successive tool hints on the same card wait for the typewriter window plus `liveToolHintMinDwellMs` so each line can typewrite (or dump via `fast` after the cap) and stay readable before the next replaces it (the card still shows one line at a time — it does not stack tools). Pulse cadence is `liveToolHintHeartbeatSeconds` (default `0.3`); pulses wait for the capped typewriter window plus glance dwell first; the effective rate is `max(interval, CardKit RTT)`. If the turn ends on the thinking line, finalize flushes `AI thinking - done`. Post-turn consolidation can reopen a live progress card after the answer freezes the previous one; `history consolidated` then freezes that card. New lines also use the typewriter config (`liveToolHintPrintFrequencyMs` / `liveToolHintPrintStep` / `liveToolHintPrintStrategy` / `liveToolHintTypewriterCapMs` / `liveToolHintMinDwellMs`). In `inline` mode these status lines are skipped (they only make sense on the live card).
