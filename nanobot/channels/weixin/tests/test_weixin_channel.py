...
@pytest.mark.asyncio
async def test_stream_end_resuming_does_not_send() -> None:
    channel, _bus = _make_channel()
    channel._client = object()
    channel._token = "token"
    channel._context_tokens["wx-user"] = "ctx-1"
    ...
    await channel.send_delta("wx-user", "hello ", {"_stream_delta": True})
    await channel.send_delta("wx-user", "world", stream_end=True, resuming=True)
...
@pytest.mark.asyncio
async def test_stream_end_resuming_then_final_still_sends_once() -> None:
    channel, _bus = _make_channel()
    channel._client = object()
    channel._token = "token"
    channel._context_tokens["wx-user"] = "ctx-1"
